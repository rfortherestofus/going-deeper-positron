...
theme_student <- function() {
  theme_minimal() +
    theme(
      ...
      plot.title = element_markdown(family = "Inter Tight"),
...
enrollment_by_race_ethnicity |>
  filter(race_ethnicity == "Hispanic/Latino") |>
  mutate(
    highlight_district = case_when(
      district == top_growth_district ~ "Y",
      .default = "N"
    )
  ) |>
  mutate(
    pct_formatted = case_when(
      highlight_district == "Y" & year == "2022-2023" ~ str_glue(
        "{pct_formatted} of students
  ...
  labs(
    title = str_glue(
      "<b style='color: orange;'>{top_growth_district}</b> saw a large growth in its Hispanic/Latino student population over the last two years"
    )
  ) +
  theme_student()

ggsave(
  filename = "enrollment-plot.png",
  width = 12,
  height = 8,
  bg = "white",
  device = agg_png
