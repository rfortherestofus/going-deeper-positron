...
- Copy the starter code below into Positron and use it to make a table using `flextable` or `gt`.
...
- Provides syntax to relatively easily adjust page size and margins
...
1. Apply my brand.yml file to my HTML file.
...
- Revealjs will apply styles set in brand.yml
...
- Put content in columns and practice using incremental reveal
...
- Applies to dashboards the same as other HTML-based formats
...
# Parameterized Reporting, Part 1 {.inverse}
...
1. Open the course project in Positron.

. . .

Make sure to install the {gapminder}, {rmarkdown}, {rstudioapi}, and {quarto} packages in order to be able to run the code in the project.
...
1. Add a parameter in your YAML for `continent` and set its value to Asia.

1. On line 29, change the code to filter the gapminder data using the `continent` parameter.

1. On line 33, use inline R code to replace Asia with your `continent` parameter.

. . .

Make sure you can render your report and have everything work exactly the same as when you first rendered.

# Parameterized Reporting, Part 2 {.inverse}
...
1. Open the file called `your-turn-render-part-1.R`.

1. Replace all instances of `TODO` with the correct text.

1. Render your report.

# Parameterized Reporting, Part 3 {.inverse}
