...
data_frame <-
  read_excel(
    path = "directory_name/file_name.xlsx",
    sheet = "name of sheet"
  )
```
...
german_speakers <-
  read_excel(
    path = "data-raw/german-and-french-speakers.xlsx",
    sheet = "German speakers"
  ) |>
  clean_names()
```
...
3. Download five recent Excel files with math proficiency scores (from 2015-2016 to 2018-2019 as well as 2021-2022) and put them in a `data-raw` folder
...
german_speakers_numeric |>
  mutate(
    total = number_of_german_speakers_2017 +
...
german_speakers_tidy <-
  german_speakers_numeric |>
  pivot_longer(cols = -state, names_to = "year", values_to = "number")
```
...
german_speakers |>
  pivot_longer(
    cols = c(
      number_of_german_speakers_2017,
      number_of_german_speakers_2018,
      number_of_german_speakers_2019
    )
  )
...
#| echo: true
data_frame |>
  mutate(variable = str_remove(variable, pattern = "text to remove"))
```
...
#| echo: true
data_frame |>
  mutate(variable = recode_values(variable, "old_value" ~ "new_value"))
```
...
```{r if_else-example}
#| echo: true
#| code-line-numbers: "3-23"
#| output-location: "slide"
german_speakers |>
  pivot_longer(cols = -state, names_to = "year", values_to = "number") |>
  mutate(
    year = if_else(
      year == "number_of_german_speakers_2017",
      true = "2017",
      false = year
    )
  ) |>
  mutate(
    year = if_else(
      year == "number_of_german_speakers_2018",
      true = "2018",
      false = year
    )
  ) |>
  mutate(
    year = if_else(
      year == "number_of_german_speakers_2019",
      true = "2019",
      false = year
    )
  )
...
1. Convert all instances of the `race_ethnicity` variable to more meaningful observations (e.g. turn "american_indian_alaska_native" into "American Indian/Alaska Native") using `recode_values()`, `if_else()`, or `case_when()`.

## Your Turn, Continued {.your-turn}

3. Add your code to the pipeline where you initially create `enrollment_by_race_ethnicity_2022_2023`. You only need to use one of the options of `recode_values()`, `if_else()`, or `case_when()` for this.
...
#| echo: false
activities_survey <-
  read_excel(here("data-raw", "google-forms-data.xlsx")) |>
  clean_names() |>
...
#| echo: true
activities_survey |>
  separate_longer_delim(cols = activities, delim = ", ") |>
  count(activities)
```
...
math_scores_2021_2022 <-
  read_excel(
    path = "data-raw/pagr_schools_math_tot_raceethnicity_2122.xlsx",
    na = c("*", "-", "--")
  ) |>
  clean_names()
```
...
1. To check that your function works, create `
enrollment_by_race_ethnicity_2021_2022
` and `
enrollment_by_race_ethnicity_2022_2023
` data frames and then bind them together with `
bind_rows()
`.
...
#| eval: true
#| echo: false
french_speakers_2019 <- read_excel(
  path = here("data-raw", "german-and-french-speakers.xlsx"),
...
#| eval: false
#| echo: true
type_of_join(x, y, join_by(id_variable))
```
...
#| eval: false
#| echo: true
semi_join(german_speakers_2019, french_speakers_2019, join_by(state))
```
